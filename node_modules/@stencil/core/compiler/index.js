...
const COMPILER_BUILD = {
    id: '20200319220704',
    minfyJs: 'terser4.6.4_6',
    optimizeCss: 'autoprefixer9.7.4_postcss7.0.27_6',
    transpiler: 'typescript3.8.3_6'
};
...
const lazyComponentTransformLegacy = (compilerCtx, transformOpts) => {
    return transformCtx => {
        return tsSourceFile => {
            const moduleFile = getModuleFromSourceFile(compilerCtx, tsSourceFile);
...
const serializeImportPath = (data) => {
    ...
    if (isString(p)) {
        if (isString(data.importerPath) && path$1.isAbsolute(data.importeePath)) {
            p = path$1.relative(path$1.dirname(data.importerPath), data.importeePath);
        }
        p = normalizePath(p);
        if (!p.startsWith('.')) {
            p = './' + p;
...
const transformToNativeComponentText = (config, compilerCtx, buildCtx, cmp, inputJsText) => {
    let outputText = null;
    try {
...
const transformToHydrateComponentText = (config, compilerCtx, buildCtx, cmp, inputJsText) => {
    let outputText = null;
    try {
        const tsCompilerOptions = {
            module: ts.ModuleKind.ESNext,
...
function generateServiceWorkerUrl(config, outputTarget, serviceWorker) {
    let swUrl = normalizePath(config.sys.path.relative(outputTarget.appDir, serviceWorker.swDest));
    if (swUrl.charAt(0) !== '/') {
        swUrl = '/' + swUrl;
    }
...
const injectRegisterServiceWorker = async (config, buildCtx, outputTarget, doc) => {
    const swUrl = generateServiceWorkerUrl(config, outputTarget, outputTarget.serviceWorker);
...
const generateMarkdown = (config, userContent, cmp, cmps, readmeOutput) => {
...
const getCssImports = (config, buildCtx, filePath, styleText) => {
    const imports = [];
...
const resolveCssNodeModule = (config, diagnostics, filePath, cssImportData) => {
    try {
        const dir = config.sys.path.dirname(filePath);
...
const getModuleLegacy = (config, compilerCtx, sourceFilePath) => {
...
function parseCollectionComponentsLegacy(config, compilerCtx, buildCtx, collectionDir, collectionManifest, collection) {
...
const parseCollectionManifestLegacy = (config, compilerCtx, buildCtx, collectionName, collectionDir, collectionJsonStr) => {
...
const parseCollectionLegacy = (config, compilerCtx, buildCtx, pkgJsonFilePath, pkgData) => {
    ...
    collection = parseCollectionManifestLegacy(config, compilerCtx, buildCtx, collectionName, collectionDir, collectionJsonStr);
