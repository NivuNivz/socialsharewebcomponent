/*
 Stencil Testing Platform v1.10.3 | MIT Licensed | https://stenciljs.com
 */
'use strict';
    ...
    if (style) {
        if (typeof style === 'string') {
            ...
            if (!appliedStyles.has(scopeId)) {
                if (appData.BUILD.hydrateClientSide && styleContainerNode.host && (styleElm = styleContainerNode.querySelector(`[${HYDRATED_STYLE_ID}="${scopeId}"]`))) {
...
const attachStyles = (hostRef) => {
    ...
    const scopeId = addStyle((appData.BUILD.shadowDom && exports.supportsShadow && elm.shadowRoot)
        ? elm.shadowRoot
...
const createElm = (oldParentVNode, newParentVNode, childIndex, parentElm) => {
    ...
    if (appData.BUILD.vdomText && newVNode.$text$ !== null) {
        ...
        elm = newVNode.$elm$ = (appData.BUILD.isDebug || appData.BUILD.hydrateServerSide) ? slotReferenceDebugNode(newVNode) : doc.createTextNode('');
    }
    else {
        ...
        elm = newVNode.$elm$ = ((appData.BUILD.svg)
            ? doc.createElementNS(isSvgMode ? SVG_NS : HTML_NS, (appData.BUILD.slotRelocation && newVNode.$flags$ & 2 /* isSlotFallback */) ? 'slot-fb' : newVNode.$tag$)
            : doc.createElement((appData.BUILD.slotRelocation && newVNode.$flags$ & 2 /* isSlotFallback */) ? 'slot-fb' : newVNode.$tag$));
...
const patch = (oldVNode, newVNode) => {
    ...
    if (!appData.BUILD.vdomText || text === null) {
        ...
        if (appData.BUILD.vdomAttribute || appData.BUILD.reflect) {
            if (appData.BUILD.slot && tag === 'slot') ;
            else {
...
render() {
  ...
  `);
    ...
    if (appData.BUILD.slotRelocation) {
        ...
        useNativeShadowDom = exports.supportsShadow && (cmpMeta.$flags$ & 1 /* shadowDomEncapsulation */) !== 0;
        // always reset
        ...
        if (checkSlotRelocate) {
            ...
            for (; i < relocateNodes.length; i++) {
                ...
                if (!nodeToRelocate['s-ol']) {
                    ...
                    orgLocationNode = (appData.BUILD.isDebug || appData.BUILD.hydrateServerSide)
...
const getElement = (ref) => appData.BUILD.lazyLoad ? getHostRef(ref).$hostElement$ : ref;
...
const emitEvent = (elm, name, opts) => {
    const ev = new (appData.BUILD.hydrateServerSide ? win.CustomEvent : CustomEvent)(name, opts);
...
const appDidLoad = (who) => {
    ...
    nextTick(() => (emitEvent(win, 'appload', { detail: { namespace: appData.NAMESPACE } })));
...
const addHydratedFlag = (elm) => appData.BUILD.hydratedClass ?
...
const initializeClientHydrate = (hostElm, tagName, hostId, hostRef) => {
    ...
    childRenderNodes.map(c => {
        ...
        const node = c.$elm$;
        if (orgLocationNode && (exports.supportsShadow && orgLocationNode['s-en'] === '')) {
...
const initializeComponent = async (elm, hostRef, cmpMeta, hmrVersionId, Cstr) => {
    ...
    if ((appData.BUILD.lazyLoad || appData.BUILD.hydrateServerSide || appData.BUILD.style) && (hostRef.$flags$ & 32 /* hasInitializedComponent */) === 0) {
        ...
        if (appData.BUILD.style && !styles.has(scopeId) && Cstr.style) {
            ...
            if (!appData.BUILD.hydrateServerSide && appData.BUILD.shadowDom && appData.BUILD.shadowDomShim && cmpMeta.$flags$ & 8 /* needsShadowDomShim */) {
    ...
    if ((plt.$flags$ & 1 /* isTmpDisconnected */) === 0) {
        ...
        if (!(hostRef.$flags$ & 1 /* hasConnected */)) {
            ...
            if (appData.BUILD.hydrateClientSide) {
                ...
                if (hostId) {
                    if (appData.BUILD.shadowDom && exports.supportsShadow && cmpMeta.$flags$ & 1 /* shadowDomEncapsulation */) {
...
const setContentReference = (elm) => {
    ...
    // create a node to represent where the original
    // content was first placed, which is useful later on
    const contentRefElm = elm['s-cr'] = doc.createComment(appData.BUILD.isDebug
...
const patchCloneNode = (HostElementPrototype) => {
    ...
    HostElementPrototype.cloneNode = function (deep) {
        const srcNode = this;
        const isShadowDom = appData.BUILD.shadowDom ? srcNode.shadowRoot && exports.supportsShadow : false;
...
const bootstrapLazy = (lazyBundles, options = {}) => {
    ...
    if (appData.BUILD.hydrateClientSide && appData.BUILD.shadowDom) {
        for (; i < styles.length; i++) {
            registerStyle(styles[i].getAttribute(HYDRATED_STYLE_ID), convertScopedToShadow(styles[i].innerHTML));
        }
    }
    ...
    if (appData.BUILD.hydratedClass || appData.BUILD.hydratedAttribute) {
